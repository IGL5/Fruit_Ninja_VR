...
public class GameManagerAgua : MonoBehaviour
{
    ...
    void Update()
    {
		if(!gameOver){
			...
			if(vidas < 3 && Random.Range(0,1000) == 1 && powerupActivo == false){
				GameObject newPowerup = Instantiate(powerup, new Vector3(0, 0, 0), powerup.transform.rotation);
				newPowerup.transform.localScale = powerup.transform.localScale * 2;
				powerupActivo = true;
			}
		}
    ...
    {
        // Seleccionar un cañón aleatorio de la lista
        Transform cañonSeleccionado = cañones[Random.Range(0, cañones.Count)];

        // Calcular la posición de la fruta
        Vector3 posicionCañon = cañonSeleccionado.position;

        // Instanciar una fruta aleatoria
        var fruta = prefabs[Random.Range(0, prefabs.Count)];
        var frutaInstanciada = Instantiate(fruta, posicionCañon, fruta.transform.rotation);
		frutaInstanciada.transform.localScale = frutaInstanciada.transform.localScale * 2;

        // Calcular la velocidad para lanzar la fruta
    ...
    public void quitaVida(){
        ...
        if(vidas == 0){
            Debug.Log("Game Over");
            gameOver = true;
            //Activar aqui el evento de game over
        }
        cambiaVidas(vidas);
        //textoVida.text = "VIDAS: " + vidas;
    }
